...
void inicializar_cuentas(sem_t *semaforo) {
    struct Cuenta cuentas[] = {
        {1001, "Juan", 5000.00, 0},    // Eliminar espacios finales
        {1002, "Marcos", 3000.00, 0},
        {1003, "Carlos", 7000.00, 0},
        {1004, "Ana", 4000.00, 0}
    };
    int num_cuentas = sizeof(cuentas) / sizeof(cuentas[0]);

    sem_wait(semaforo); // Bloquear acceso al archivo
    FILE *archivo = fopen("data/cuentas.dat", "wb"); // Cambiar a "wb" para sobrescribir
    if (!archivo) {

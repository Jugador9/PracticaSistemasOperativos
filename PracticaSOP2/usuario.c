...
/* Definimos la estructura para los mensajes de la cola, la cuenta, la tabla de cuentas en la memoria compartida, las operaciones y la configuración*/
struct msgbuf {
    long tipo;
    char texto[256];
};
...
/* Desarrollamos la función para leer la configuración desde config.txt*/
Config leer_configuracion(const char *ruta) {
    Config configuracion = {0}; // Inicializar para evitar valores no definidos
...
/* Desarrollamos la función que muestra el menú principal del sistema bancario*/
...
/* Desarrollamos la función para registrar transacciones en un log por usuario y enviar a la cola de mensajes*/
void registrar_transaccion(int tipo, int num_cuenta, float monto, int num_cuenta_destino, int msqid) {
    ...
    if (stat("data", &st) == -1) {
        if (mkdir("data", 0700) == -1) {
            perror("Error al crear directorio data");
            return;
        }
    }
...
/* Desarrollamos la función que realiza la operación bancaria usando memoria compartida*/
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (pthread_create(&hilo_operacion, NULL, realizar_operacion, op) != 0) {

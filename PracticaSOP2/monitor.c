...
/* Definimos la estructura para mensajes de la cola y operaciones recientes*/
struct msgbuf {
    long tipo;
    char texto[MAX_LINE];
};
...
/* Desarrollamos la función que elimina los espacios iniciales de una cadena*/
...
/* Desarrollamos la función que elimina espacios y caracteres finales (\n) de una cadena*/
...
/* Desarrollamos la función que analiza una línea de transacción y genera alertas si es necesario*/
void analizar_linea(char *linea, int pipe_write_fd, int limite_retiro, int limite_transferencia, int umbral_retiros, int umbral_transferencias) {
    ...
    // Extraemos cuenta y monto
    int cuenta = 0, cuenta_destino = 0;
    float monto = 0.0;
    if (strcmp(operacion, "Transferencia") == 0) {
        // Parseamos transferencia con formato más flexible
    ...
    // Generamos alertas para límites
    char alerta[256];
    ...
    // Contamos retiros y transferencias
    int retiros[10000] = {0};
    ...
    // Generamos alertas para umbrales
